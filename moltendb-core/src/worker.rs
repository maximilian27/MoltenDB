...
impl WorkerDb {
    ...
    fn handle_get(&self, request: &Value) -> Result<JsValue, JsValue> {
        let (code, mut body): (u16, serde_json::Value) = handlers::process_get::process_get(&self.db, request, 5 * 1024 * 1024);
    ...
    fn handle_set(&self, request: &Value) -> Result<JsValue, JsValue> {
        let (code, mut body): (u16, serde_json::Value) = handlers::process_set::process_set(&self.db, request, 5 * 1024 * 1024);
        if let Some(obj) = body.as_object_mut() { obj.insert("statusCode".into(), serde_json::json!(code)); }
        if code >= 400 { return Err(serde_wasm_bindgen::to_value(&body).map_err(|e| JsValue::from_str(&e.to_string()))?); }
        self.maybe_compact();
        serde_wasm_bindgen::to_value(&body).map_err(|e| JsValue::from_str(&e.to_string()))
    ...
    fn handle_update(&self, request: &Value) -> Result<JsValue, JsValue> {
        let (code, mut body): (u16, serde_json::Value) = handlers::process_update::process_update(&self.db, request, 5 * 1024 * 1024);
        if let Some(obj) = body.as_object_mut() { obj.insert("statusCode".into(), serde_json::json!(code)); }
        if code >= 400 { return Err(serde_wasm_bindgen::to_value(&body).map_err(|e| JsValue::from_str(&e.to_string()))?); }
        self.maybe_compact();
        serde_wasm_bindgen::to_value(&body).map_err(|e| JsValue::from_str(&e.to_string()))
    ...
    fn handle_delete(&self, request: &Value) -> Result<JsValue, JsValue> {
        let (code, mut body): (u16, serde_json::Value) = handlers::process_delete::process_delete(&self.db, request, 5 * 1024 * 1024);
        if let Some(obj) = body.as_object_mut() { obj.insert("statusCode".into(), serde_json::json!(code)); }
        if code >= 400 { return Err(serde_wasm_bindgen::to_value(&body).map_err(|e| JsValue::from_str(&e.to_string()))?); }
        self.maybe_compact();
        serde_wasm_bindgen::to_value(&body).map_err(|e| JsValue::from_str(&e.to_string()))
