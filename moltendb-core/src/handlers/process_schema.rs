...
pub fn process_schema(db: &engine::Db, payload: &Value, max_body_size: usize, max_keys_per_request: usize) -> (u16, Value) {
    ...
    const SCHEMA_ALLOWED: &[&str] = &["collection", "schema", "ttl"];
    ...
    // At least one of "schema" or "ttl" must be provided.
    if payload.get("schema").is_none() && payload.get("ttl").is_none() {
        return (400, json!({ "error": "At least one of 'schema' or 'ttl' must be provided", "statusCode": 400 }));
    }
