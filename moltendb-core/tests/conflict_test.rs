...
async fn test_insert_batch_conflict() {
    ...
    db.insert("users", vec![("u1".to_string(), json!({"name": "Alice"}))])
        .unwrap();
    let doc = db
    ...
    db.insert(
        "users",
        vec![("u1".to_string(), json!({"name": "Alice Updated"}))],
    )
    .unwrap();
    let doc = db
        .get("users", vec!["u1".to_string()])
    ...
    db.insert(
        "users",
        vec![("u1".to_string(), json!({"name": "Alice v3"}))],
    )
    .unwrap();
    let doc = db
        .get("users", vec!["u1".to_string()])
...
async fn test_update_conflict_guard() {
    ...
    let res = db
        .update("users", "u1", json!({"role": "admin", "_v": 1}))
        .unwrap();
    assert!(res);
    let doc = db
        .get("users", vec!["u1".to_string()])
