...
fn open_db() -> engine::Db {
    ...
    engine::Db::open(path.to_str().unwrap(), true, false, 50000, None).expect("open db")
}
...
fn test_persistence_survives_reopen() {
    ...
    let _ = std::fs::remove_file(path);
    {
        let db = engine::Db::open(&path, true, false, 50000, None).unwrap();
        set(&db, json!({
            "collection": "items",
            "data": { "k1": { "value": 42 } }
        }));
    ...
    let db2 = engine::Db::open(&path, true, false, 50000, None).unwrap();
...
fn test_concurrent_writes() {
    ...
    let path = path_buf.to_str().unwrap();
    let _ = std::fs::remove_file(path);
    let db = Arc::new(engine::Db::open(&path, true, false, 50000, None).unwrap());
    let n_threads = 8;
    let n_docs = 100;
    let mut handles = vec![];
